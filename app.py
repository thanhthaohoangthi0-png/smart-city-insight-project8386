...
# --- 6. PHẦN CHATBOT (SỬA LỖI TRUY XUẤT) ---
st.divider()
st.subheader("🤖 Trợ lý AI (Hệ thống phân tích chuyên sâu)")

# Sử dụng mã từ Secrets (Không dán mã thật vào đây)
try:
    genai.configure(api_key=st.secrets["GOOGLE_API_KEY"])
except Exception as e:
    st.error("Chưa cấu hình GOOGLE_API_KEY trong Secrets của Streamlit.")
...
if model is None:
    st.warning("⚠️ Không tìm thấy mô hình AI nào khả dụng. Vui lòng kiểm tra lại API Key trong Secrets.")
else:
